...
class Table(object):
    def config_db(self,pkg):
        ...
        tbl.column('arrival_id',size='22', name_long='arrival_id'
                    ).relation('arrival.id', relation_name='cargo_lu_arr', mode='foreignkey', onDelete='cascade',onDuplicate=False)
        tbl.column('bln', size=':20', name_short='BL no.')
        tbl.column('bl_date', dtype='D', name_short='!![en]B/L date')
        ...
        tbl.formulaColumn('cargo_lu_en_ita', """CASE WHEN $operation = 'L' THEN '-Carico da imbarcare / Loading cargo: ' || ' ' || @measure_id.description || ' ' || $quantity || ' ' || $description_it || '<br>' 
                                            WHEN $operation = 'U' THEN '-Carico da sbarcare / Unloading cargo: ' || @measure_id.description || ' ' || to_char($quantity,:df) || ' ' || $description_it || '<br>' ELSE 'Nessun carico da movimentare' END """,
                            var_df='999999D999',dtype='T', name_long='Carico L/U ITA')
