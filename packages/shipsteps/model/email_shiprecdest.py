...
class Table(object):
    def config_db(self,pkg):
        tbl=pkg.table('email_shiprecdest', pkey='id', name_long='!![en]Email std shiprec q.ty dest.', name_plural='!![en]Email std shiprec q.ty dest.',caption_field='description')
        self.sysFields(tbl,counter=True)
        tbl.column('ship_rec_id',size='22', name_long='ship_rec_id'
                    ).relation('ship_rec.id', relation_name='email_shiprec_dest', mode='foreignkey', onDelete='cascade')
