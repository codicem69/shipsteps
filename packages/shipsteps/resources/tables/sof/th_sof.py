...
class View(BaseComponent):
    ...
    def th_struct(self,struct):
        ...
        r.fieldcell('et_start', width='6em')
        r.fieldcell('etc', width='6em')
        r.fieldcell('nor_tend', width='6em')
        r.fieldcell('nor_rec', width='6em')
        r.fieldcell('nor_acc', width='7em')
        r.fieldcell('customs_commenced', width='6em')
        r.fieldcell('customs_completed', width='6em')
        r.fieldcell('ops_commenced', width='6em')
        r.fieldcell('ops_completed', width='6em')
        r.fieldcell('doc_onboard', width='6em')
        r.fieldcell('ship_rec', width='40em')
        r.fieldcell('intestazione_sof',width='auto')
...
class Form(BaseComponent):
    ...
    def checkEmail(self,record=None,rec_id=None,**kwargs):
        ...
        emailsof=self.db.table('shipsteps.email_sof').query(columns='$id,$description,$email', where='$sof_id=:sof_id',sof_id=rec_id).fetch() 
        dati=[]
        for r in emailsof:
...
#
    ...
    def dailyOperations(self,bc_daily):
        ...
        bc_dqt=bc_daily.roundedGroupFrame(title='!![en]Daily quantity handled',width='40%', height='100%',region='left',splitter=True, closable=True).inlineTableHandler(maintable='shipsteps.sof',relation='@sof_daily',viewResource='ViewFromSofDailyOp',liveUpdate=True,view_store__onBuilt=True,_virtual_column="$totcargo")#.contentPane(width='100%', height='100%').inlineTableHandler(relation='@sof_daily',viewResource='ViewFromSofDailyOp')#, view_grid_autoSelect=True)
